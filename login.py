...
#=========Background Color======
 ...
 #==========Frame==============
       ...
       title=Label(login_frame,text="LOGIN HERE",font=("times new roman",38,"bold"),bg="white",fg="#08A3D2").place(x=250,y=50)
    ...
    def forget_password(self):
        if self.cmb_quest.get()=="Select"or self.txt_answer.get()=="Select" or self.txt_new_pass.get()=="":
            messagebox.showerror("Error","All fields are required",parent=self.root2)
        else:
            try:
               con=sqlite3.connect(database="rms.db")
               cur=con.cursor()
               cur.execute("select * from employee where email=? and question=? and answer=?",(self.txt_email.get(),self.cmb_quest.get(),self.txt_answer_pass.get()))
               row=cur.fetchone()
               if row==None:
        ...
        if self.txt_email.get()=="":
            ...
            try:
               ...
               if row==None:
                    ...
                    t=Label(self.root2,text="Forget password",font=("time new roman",20,"bold"),bg="white",fg="red").place(x=0,y=10,relwidth=1)
           ...
           # ------------------------Forget Password
                    ...
                    btn_change_password=Button(self.root2,text="Reset password",command=self.forget_password,bg="green",fg="white",font=("times new roman",15,"bold")).place(x=90,y=350)
    ...
    def login(self):
           if self.txt_email.get()=="" or self.txt_pass_.get()=="":
              messagebox.showerror("Error","All fiels are required",parent=self.root)
               ...
               try:
                   con=sqlite3.connect(database="rms.db") # type: ignore
                   cur=con.cursor()
